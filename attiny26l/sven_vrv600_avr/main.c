...
int main(void)
{
	...
	*cnd = cond;
    ...
    do {
        ...
        *Lin = adc_temp;
        if ((adc_temp>=V180) && (adc_temp<=V260)){
            while(1){
                adc_temp2 = getWaveMax(readADC(0)); //LineOut
                if (adc_temp2!=UNDEF){
                    break;
                }
            }
            if (adc_temp>adc_temp2){
                PORTB |= _BV(RELE1) | _BV(RELE2); //rele1 - V1, rele2- V3
#ifdef DEBUG_S
                ddelay_ms(500);
#else
                _delay_ms(500);
                ...
                *Lin = adc_temp;
                if ((adc_temp>=V180) && (adc_temp<=V260)){
                    if (adc_temp<adc_temp2){
                        //all is good no stab
                        PORTB &= ~(_BV(RELE2)); //rele1 - V1, rele2 - V2
                    } else {
                        PORTB &= ~(_BV(RELE1) | _BV(RELE2)); //rele1 - V3, rele2- V2
                    }
                } else {
                    PORTB &= ~(_BV(RELE1) | _BV(RELE2)); //rele1 - V3, rele2- V2
                }

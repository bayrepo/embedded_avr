...
void LCD_PUTC(byte x, byte y, byte ch){
    ...
    if(ch<=0x5C) textL=0x20;
	else if (ch<0x90) textL=0x21;
    else textL=0x60;
	
    for(i=0; i<5; i++){
        LCD_PUT_BYTE(x+i, y, pgm_read_byte(&(sym[ch-textL][i])));
        } 
    //Для раздиления символов
    LCD_PUT_BYTE(x+i, y, 0x00);
}   
/*Функция вывода стоки из флешь памяти*/            

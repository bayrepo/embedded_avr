...
static uint16_t rx_pulse_width_inc = 1;
...
static void RXInit() {
  ...
  //setup timers depending on the microcontroller used
    ...
    TCCR1A = 0; // reset counter on match
    TCCR1B =  _BV(CS11) | _BV(WGM12); // 1/8 prescaler
    #if F_CPU == 1000000UL
...
void MAN_RX_INTERRUPT_HANDLER() {
  if (rx_mode < RX_MODE_MSG) {//receiving something
    ...
    if (directRxPort && directRxMask) { //use direct port manipulation (much faster)
	    ...
	    rx_sample = PIND & directRxMask; 

    }


	if (rx_sample != rx_last_sample && rx_pulse_width>5) { //pin has changed
      isranie++;
